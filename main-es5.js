...
(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["main"], {
  ...
  function srcAppAppComponentTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var AppComponent = /*#__PURE__*/function () {
      ...
      _createClass(AppComponent, [{
        key: "groundInit",
        value: function groundInit() {
          var circleGeometry = new three__WEBPACK_IMPORTED_MODULE_2__["CircleGeometry"](window.innerWidth);
          var circleMaterial = new three__WEBPACK_IMPORTED_MODULE_2__["MeshBasicMaterial"]({
            color: 0x838581
          });
        ...
        key: "orbitControlsInit",
        value: function orbitControlsInit() {
        ...
        key: "init",
        value: function init() {
          this.scene = new three__WEBPACK_IMPORTED_MODULE_2__["Scene"](); // this.scene.fog = new Fog(0xCBCBCB, 200, 700);
        ...
        value: function animate() {
          ...
          if (this.bool) {
            if (Math.abs(this.currentAngle) >= this.currentMaxAngle) {
              this.aDelta = 0; // this.aDelta = -this.aDelta;
        ...
        key: "createBall",
        value: function createBall(startSpeed, startAngle, horizontalAngle, startPosition, speedRecoveryCoefficient) {
          var _this = this;
          ...
          var ball = new _ball__WEBPACK_IMPORTED_MODULE_4__["Ball"](this.ballsIDCounter, this.ballRadius, startSpeed, startAngle, startPosition, speedRecoveryCoefficient, this.controls.airResistance, sphere, this.scene, horizontalAngle, {
            length: this.controls.wallLength,
            height: 2,
            width: this.controls.wallDepth
          });
          this.scene.add(sphere);
          this.balls.push(ball);
          setTimeout(function () {
            _this.balls.splice(_this.balls.indexOf(ball), 1);
        ...
        value: function ngOnInit() {
          ...
          this.scene.add(zhelob); // this.start();
          // setInterval(() => {
  ...
  /***/
  function srcAppBallTs(module, __webpack_exports__, __webpack_require__) {
    "use strict";
    ...
    var Ball = /*#__PURE__*/function () {
      function Ball(id, radius, startSpeed, startAngle, startPosition, speedRecoveryCoefficient, airResistance, object3D, scene, horizontalAngle, plane) {
        ...
        var pointGeometry = new three__WEBPACK_IMPORTED_MODULE_1__["SphereGeometry"](3, 50);
        var pointMaterial = new three__WEBPACK_IMPORTED_MODULE_1__["MeshBasicMaterial"]({
          color: 0xFFFFFF
        });
        this.point = new three__WEBPACK_IMPORTED_MODULE_1__["Mesh"](pointGeometry, pointMaterial); // this.scene.add(this.point);
      ...
      _createClass(Ball, [{
        ...
        value: function getPlane() {
          var wall = this.scene.getObjectByName("wall");
          var currentVertAngle = wall.rotation.z;
          var currentHorizAngle = wall.rotation.y;
          var point_0 = wall.position;
          var point_1 = new three__WEBPACK_IMPORTED_MODULE_1__["Vector3"](wall.position.x + this.plane.length * Math.cos(currentVertAngle) * Math.cos(currentHorizAngle), wall.position.y + this.plane.length * Math.sin(currentVertAngle), wall.position.z - this.plane.length * Math.cos(currentVertAngle) * Math.sin(currentHorizAngle));
          var point_2 = new three__WEBPACK_IMPORTED_MODULE_1__["Vector3"](wall.position.x - this.plane.width * Math.cos(Math.PI / 2 - currentHorizAngle), wall.position.y, wall.position.z - this.plane.width * Math.sin(Math.PI / 2 - currentHorizAngle));
          var xK = (point_1.y - point_0.y) * (point_2.z - point_0.z) - (point_2.y - point_0.y) * (point_1.z - point_0.z);
          var yK = (point_1.x - point_0.x) * (point_2.z - point_0.z) - (point_2.x - point_0.x) * (point_1.z - point_0.z);
          var zK = (point_1.x - point_0.x) * (point_2.y - point_0.y) - (point_2.x - point_0.x) * (point_1.y - point_0.y);
          var D = -point_0.x * xK + -point_0.y * yK + -point_0.z * zK;
          return {
            xK: xK,
            yK: yK,
            zK: zK,
        ...
        key: "checkCollision2",
        value: function checkCollision2() {
          var wall = this.scene.getObjectByName("wall");
          var points = [// this.currentPos(),
          this.object3D.position, new three__WEBPACK_IMPORTED_MODULE_1__["Vector3"](this.object3D.position.x + this.radius, this.object3D.position.y, this.object3D.position.z), // new Vector3(this.object3D.position.x - this.radius, this.object3D.position.y, this.object3D.position.z),
          ...
          var currentVertAngle = wall.rotation.z;
          var currentHorizAngle = wall.rotation.y;
          var leftX = wall.position.x - this.plane.height - Math.abs(this.plane.length * Math.cos(currentVertAngle) * Math.cos(currentHorizAngle)) - Math.abs(this.plane.width * Math.cos(currentVertAngle) * Math.sin(currentHorizAngle));
          var rightX = wall.position.x + this.plane.height + Math.abs(this.plane.length * Math.cos(currentVertAngle) * Math.cos(currentHorizAngle)) + Math.abs(this.plane.width * Math.cos(currentVertAngle) * Math.sin(currentHorizAngle));
          var leftY = wall.position.y - this.plane.height - Math.abs(this.plane.length * Math.sin(currentVertAngle));
          var rightY = wall.position.y + this.plane.height + Math.abs(this.plane.length * Math.sin(currentVertAngle));
          var leftZ = wall.position.z - Math.abs(this.plane.width * Math.cos(currentHorizAngle)) - Math.abs(this.plane.length * Math.sin(currentHorizAngle));
          var rightZ = wall.position.z + Math.abs(this.plane.width * Math.cos(currentHorizAngle)) + Math.abs(this.plane.length * Math.sin(currentHorizAngle));
          var result = this.getPlane(); // tslint:disable-next-line:prefer-for-of

          for (var i = 0; i < points.length; i += 1) {
            var onX = points[i].x >= leftX && points[i].x <= rightX;
            var onY = points[i].y >= leftY && points[i].y <= rightY;
            var onZ = points[i].z >= leftZ && points[i].z <= rightZ;
            var distanceToPlane = Math.abs(result.xK * points[i].x + result.yK * points[i].y + result.zK * points[i].z + result.D) / Math.sqrt(Math.pow(result.xK, 2) + Math.pow(result.yK, 2) + Math.pow(result.zK, 2));
        ...
        value: function newPosition() {
          return new three__WEBPACK_IMPORTED_MODULE_1__["Vector3"](this.object3D.position.x + this.startSpeed * Math.cos(this.startAngle * this.degToRad) * Math.cos(this.horizontalAngle * this.degToRad) * this.step - this.airResistance / this.weight * this.startSpeed * this.startSpeed * Math.cos(this.startAngle * this.degToRad) * Math.cos(this.horizontalAngle * this.degToRad) / 2 * Math.pow(this.step, 2), this.object3D.position.y + this.startSpeed * Math.sin(this.startAngle * this.degToRad) * this.step - (9.81 + this.airResistance / this.weight * this.startSpeed * this.startSpeed * Math.sin(this.startAngle * this.degToRad) / 2) * Math.pow(this.step, 2), this.object3D.position.z + this.startSpeed * Math.cos(this.startAngle * this.degToRad) * Math.sin(this.horizontalAngle * this.degToRad) * this.step - this.airResistance / this.weight * this.startSpeed * this.startSpeed * Math.cos(this.startAngle * this.degToRad) * Math.sin(this.horizontalAngle * this.degToRad) / 2 * Math.pow(this.step, 2));
        }
      }, {
        key: "newAngle",
        value: function newAngle(nextPosition) {
          var deltaXp = nextPosition.x - this.object3D.position.x;
          var deltaY = nextPosition.y - this.object3D.position.y;
          var deltaZ = nextPosition.z - this.object3D.position.z;
          var deltaX = Math.sign(deltaXp) * Math.sqrt(Math.pow(deltaXp, 2) + Math.pow(deltaZ, 2));
        ...
        value: function newSpeed() {
          var newVx = this.startSpeed * Math.cos(this.startAngle * this.degToRad) * Math.cos(this.horizontalAngle * this.degToRad) - this.airResistance / this.weight * this.startSpeed * this.startSpeed * Math.cos(this.startAngle * this.degToRad) * Math.cos(this.horizontalAngle * this.degToRad) * this.step;
          var newVy = this.startSpeed * Math.sin(this.startAngle * this.degToRad) - (9.81 + this.airResistance / this.weight * this.startSpeed * this.startSpeed * Math.sin(this.startAngle * this.degToRad)) * this.step;
          var newVz = this.startSpeed * Math.cos(this.startAngle * this.degToRad) * Math.sin(this.horizontalAngle * this.degToRad) - this.airResistance / this.weight * this.startSpeed * this.startSpeed * Math.cos(this.startAngle * this.degToRad) * Math.sin(this.horizontalAngle * this.degToRad) * this.step;
  ...
  function _(module, exports, __webpack_require__) {
    ...
    /*! N:\workspaces\webstorm-workspace\niku-niku\src\main.ts */
    "./src/main.ts");
    /***/
  }
}, [[0, "runtime", "vendor"]]]);
